...
export default () => {
  ...
  if (!PLAYER_BLOCK) return errorManagement({ errCase: 'elementLoss', message: 'cardHideAnimationComn 에서 .player-block 엘리먼트가 없습니다' });
  const PLAYER_CARD = PLAYER_BLOCK.querySelector('.player-card');
  if (!PLAYER_CARD) return errorManagement({ errCase: 'elementLoss', message: 'cardHideAnimationComn 에서 .player-card 엘리먼트가 없습니다' });
  const ENEMY_BLOCK = document.querySelector('.enemy-block');
  if (!ENEMY_BLOCK) return errorManagement({ errCase: 'elementLoss', message: 'cardHideAnimationComn 에서 .enemy-block 엘리먼트가 없습니다' });
  const ENEMY_COINS = ENEMY_BLOCK.querySelector('.coins-enemy');
  if (!ENEMY_COINS) return errorManagement({ errCase: 'elementLoss', message: 'cardHideAnimationComn 에서 .coins-enemy 엘리먼트가 없습니다' });
  const ENEMY_CARD = ENEMY_BLOCK.querySelector('.enemy-card');
  if (!ENEMY_CARD) return errorManagement({ errCase: 'elementLoss', message: 'cardHideAnimationComn 에서 .enemy-card 엘리먼트가 없습니다' });
