...
export default () => {
  ...
  if (encryptVal1 === null || (encryptVal1 !== null && encryptVal1 === '')) return errorManagement({ errCase: 'sessionStorageLoss', message: 'game over 상태에서 ani 중 result 세션이 없습니다' });
  ...
  setTimeout(() => {
    ...
    if (decryptVal1) PLAYER_BLOCK.classList.add('win');
    if (!decryptVal1) ENEMY_BLOCK.classList.add('win');
    setTimeout(() => {
      LOADING_EVENT.hide();
      drewResultInfo();
      setTimeout(() => {
        // if (!RESULT_RES) if (COINS_ENEMY) COINS_ENEMY.remove();
        if (!decryptVal1) if (COINS_ENEMY) COINS_ENEMY.remove();
      }, timeInterval_1190);
    }, timeInterval_1);
  }, timeInterval_1);
};
