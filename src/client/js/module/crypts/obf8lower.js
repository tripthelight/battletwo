...
const A="abcdefghijklmnopqrstuvwxyz0123456789", L=36n;
const enc36=q=>{let n=BigInt(q),s="";do{s=A[Number(n%L)]+s;n/=L}while(n>0n);for(;s.length<8;)s=A[0]+s;return s};
const dec36=s=>{let n=0n;for(const c of s)n=n*L+BigInt(A.indexOf(c));return n};
const fnv1a=s=>{let x=2166136261>>>0;for(let i=0;i<s.length;i++){x^=s.charCodeAt(i);x=Math.imul(x,16777619)>>>0}return x>>>0};
const mix=x=>{x^=x>>>16;x=Math.imul(x,0x7feb352d)>>>0;x^=x>>>15;x=Math.imul(x,0x846ca68b)>>>0;x^=x>>>16;return x>>>0};
const rnd=()=>{try{const u=new Uint32Array(1);crypto.getRandomValues(u);return u[0]>>>0}catch(_){return (Math.random()*4294967296)>>>0}};
...
export function dec(tok){
  ...
  const K=ensureKey();
  const x=dec36(tok), t=Number(x&15n),
